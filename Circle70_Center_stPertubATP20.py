...
T = 1000 # final time
num_step = 4000 # number of time step
dt = T / num_step

## import mesh
msh = meshio.read("Circle70.msh")
...
radius_influx = 10.
subdomain = Expression('(pow(x[0],2)+pow(x[1]-70,2)) < (r * r) ? 1. : 0', r=radius_influx, degree=1)
...
print('influx', influx)
f_1 = Expression('(pow(x[0],2)+pow(x[1]-70,2)) < (r * r) ? influx : 0', influx=influx, r=radius_influx, degree=1)
...
radius_outflux = 10.
subdomain_outflux = Expression('(pow(x[0]- 140,2)+pow(x[1]- 70,2)) < (r * r) ? 1. : 0', r=radius_outflux, degree=1)
subdomain_outflux_area = assemble(subdomain_outflux * dx(mesh))

outflux = 0.0969 * area / subdomain_outflux_area

eta_f = Expression('(pow(x[0] - 140,2)+pow(x[1]- 70.,2)) < (r * r) ? outflux : 0', outflux = outflux, r=radius_outflux, degree=1)
...
np.save('./output_Circle70_Center_stPertubATP20', np.asarray(list_of_list))
